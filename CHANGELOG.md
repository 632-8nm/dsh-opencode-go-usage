...
### 新增(秒开 + 持久化)
- **官方最近会话标题修复**:提取 rebuildOfficialRecent——DSH 扫描未就绪时官方 recent 置空(不再退化为官方记录聚合显示一片"(无标题)");扫描完成即重建真实标题,不再等 60s 轮询
- **配额快照兜底**:任何构建路径(含重启后后台刷新抢占构建)配额缓存为空时,自动用磁盘快照配额打底——面板配额区永不空白,后台抓取后 60s 轮询更新
- **手动刷新强制重抓**:刷新按钮跳过 45s 内存缓存与快照分支,立即重建最新数据(官方 loading 中也不再"点了没反应")
...
### 新增(对标同类工具调研:OpenUsage / opencode-bar / AIUsageTracker / LiteLLM 等)
