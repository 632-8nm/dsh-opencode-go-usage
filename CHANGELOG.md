...
- 实测定位:Edge 冷启动 + 加载 profile 需要 **10-20s**,而 9222 监听验证只有 ~6s——插件提前报 NO_LISTEN,浏览器其实随后正常起来(用户环境实测:报错后 Edge 最终监听,CDP 提取成功)
- **验证窗口 6s → ~20s**(26 次 × 750ms);错误提示改为引导"稍等 20 秒后刷新 / 再点一次启动按钮",不再让用户去找 bat 文件
...
### 行为(官方视图不再秒开缓存旧数据)
- **磁盘缓存降级为纯增量基准**:此前重启后官方视图"秒开"磁盘缓存旧数据、后台增量慢慢追;现在**真实数据到位前官方视图显示"加载中"**,与 DSH 视图一致——有磁盘缓存时走增量(1-3s 拿到最新完整集,比全量快),无缓存才全量(10-15s);数据到位立即展示,不展示可能过期的缓存旧数据
- **竞态兜底**:后台抓取可能早于响应构建完成(其缓存同步未命中刚创建的响应对象)——fetchAll 现在用最新 officialCache 覆盖 loading 占位,保证数据到位后立即展示而非等下一个轮询周期
- 加载中文案通用化:"官方数据加载中(增量/全量抓取)…"

### 测试
- 新增用例:有磁盘缓存时首响应不得携带旧记录、增量(带 LAST)完成后展示真实抓取结果;截断重建用例同步更新

## [1.6.17] - 2026-08-16

### 修复(DSH 视图首次显示 0 + 官方/本地对账误导)
- **DSH 视图不再显示 0**:v1.6.15 会话扫描后台化后,首次(或重扫期间)响应里的 dsh 是旧/空数据,DSH 视图会渲染成"0 金额/0 请求"误导。现在响应带 `dshLoading` 标志,未就绪时 DSH 视图显示"会话扫描中(约 10-60 秒)"提示,**不渲染 0 数据**;客户端 15s fast-poll 自动追,扫描完成立即切换为真实数据;FAB 在 DSH 未就绪时也不显示 $0.00
- **对账行不再标红误报**:实测配额接口 monthly 33%×$60=$19.80,而同一滚动窗口(8/14 14:54 起,resetsAt 证明)的 usage.list 美元明细仅 $9.71——配额按**用量单位**计(deepseek-v4-flash 限时 2×),与逐请求美元明细不是同一计量基准,旧逻辑按 ±15% 判 warn 恒误报。现改为中性信息展示,文案与 title 说明口径差异
- 扫描失败不再静默:写入诊断日志(`dsh scan failed: ...`)

## [1.6.16] - 2026-08-16

### 功能(FAB 实时刷新)
- **关闭面板时 FAB 也 60s 刷新**:此前轮询仅在面板打开时进行(关闭时 FAB 金额/配额百分比静止)。v1.6.15 起会话扫描已后台化、官方增量轻量、配额秒级,关闭状态 60s 一次的开销可控——现在**面板开/关都保持 60s 轮询**,FAB 的累计金额与滚动配额百分比始终实时
- 15s fast-poll(官方明细首次拉取)仍只在面板打开时启用;关闭时 60s 常规轮询自然覆盖
