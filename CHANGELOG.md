...
### 凭据指引
- 明确 auth Cookie 只能复制 Value,不能复制 `auth=` 前缀或整条 Cookie header
- 明确 workspaceId 的完整 URL 示例和 `wrk_xxx` 格式
- 保存配置时校验 workspaceId 格式,错误输入立即返回可读提示
- 新增手动凭据格式回归测试,测试增至 28 个
...
### 安全与缓存策略
- 配置端点增加 32KB 请求体上限,避免异常大请求占用内存
- Host 数据路由和客户端取数统一使用 `no-store`,避免用量与配置响应被浏览器缓存
- 新增配置端点请求体边界回归测试,测试增至 27 个

## [1.6.31] - 2026-08-17

### 修复
- “重试提取”和截断缓存重建现在会传递 `OCGO_FORCE`,真正绕过 Python 端 15 分钟磁盘缓存,确保用户主动重试会重新抓取官方数据

## [1.6.30] - 2026-08-17

### 数据可靠性与跨平台健壮性
- 官方明细增加记录结构校验,非法 payload 不再进入统计视图
- 官方缓存增加 schema 标记,缓存/配置改为临时文件 + 原子替换,并在可用平台收紧权限
...
### 修复与健壮性
- 动态模式手动保存凭据改走 `ocgo-usage:config` Host RPC,不再依赖 `webServer` 必须存在
- 官方配置写入后尝试设置本地文件权限 `600`,并统一校验保存参数
- Windows 调试浏览器改为直接启动并验证,失败后才使用 Explorer 兜底,避免同一 profile 并发启动两次
- README 版本、测试数量、安装路径和凭据/网络隐私说明同步当前实现
