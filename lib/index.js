...
export function apply(ctx) {
    ...
    async function collectOfficial() {
      if (officialCache && officialCache.data.ok && Date.now() - officialCache.at < 15 * 60 * 1000) return officialCache.data
    ...
    async function fetchAll() {
      ...
      inflight = (async () => {
        ...
        // 失败缓存(ok:false)不算数据:collectOfficial 内部按 60s 冷却去重,
        // 避免 fast-poll 期间反复全量重试;错误原样透传给面板展示。
