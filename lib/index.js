...
export function apply(ctx) {
  ...
  "gemini-2.5-flash": { in: 0.3, out: 2.5, cr: 0.03, cw: 0.0 },
    ...
    async function collectDshScan() {
      ...
      const snaps = await mapLimit(sessions, 12, (rec) => sq.readSession(rec.header.id).catch(() => null))
    ...
    const OFFICIAL_SCRIPT = [
      ...
      'if not os.environ.get("OCGO_LAST_TS") and not os.environ.get("OCGO_START_PAGE"):',
      '    _d = load_disk_cache()',
      ...
      'START_PG = int(os.environ.get("OCGO_START_PAGE") or "0")',
      'ENV_MAXP = os.environ.get("OCGO_MAX_PAGES") or ""',
      'try:',
      '    MAXP = min(int((cfg or {}).get("maxPages", 150)) if cfg else 150, int(ENV_MAXP) if ENV_MAXP else 9999)',
      '    page = START_PG',
      ...
      '    # 仅非分段模式落盘(段1/段2 为部分数据,由 host 合并后统一写盘)',
      '    if not os.environ.get("OCGO_START_PAGE") and not os.environ.get("OCGO_MAX_PAGES"):',
    ...
    async function runOfficial(envs) {
      const envPart = (envs || []).map((e) => ";os.environ['" + e[0] + "']='" + e[1] + "'").join('')
      const cmd = "$py='E:\\python\\python.exe';if(-not(Test-Path $py)){$c=Get-Command python -ErrorAction SilentlyContinue;if($c){$py=$c.Source}else{Write-Error 'python-not-found';exit 1}}; & $py -c \"import base64,os" + envPart + ";exec(base64.b64decode('" + OFFICIAL_PAYLOAD + "'))\""
    ...
    // 合并两段抓取记录(新在前),按 (ts,model,cost,tokens) 去重
    function mergeOfficialRecords(newer, older) {
      const seen = new Set()
      const out = []
      for (const r of newer.concat(older)) {
        const k = r.ts + '|' + r.model + '|' + r.cost + '|' + (r.ti || 0) + '|' + (r.to || 0) + '|' + (r.rt || 0) + '|' + (r.cr || 0)
    ...
    // host 侧写盘(bundle 形态注入 fs;段2 合并完成后统一落盘)
    function writeOfficialDisk(records) {
      const p = officialDiskPath()
      if (!p || typeof _ocgoWriteFileSync !== 'function') return
      try {
        _ocgoMkdirSync(_ocgoJoin(_ocgoHomedir(), '.config'), { recursive: true })
        _ocgoWriteFileSync(p, JSON.stringify({ at: Date.now(), records }), 'utf8')
      } catch (e) { /* 写盘失败静默:内存缓存仍可用 */ }
    }

    // 最近一次 DSH 会话扫描结果(复用:官方段2/增量完成后仅做内存回填,不重扫)
    let lastScan = null
    ...
    async function collectOfficial() {
      ...
      officialInflight = (async () => {
        try {
          // 段1:先抓最近 40 页(约 2000 条,覆盖今天/昨天),3-6s 返回,面板先出数据
          const p1 = await runOfficial([['OCGO_MAX_PAGES', '40']])
          if (!p1 || !p1.ok) return { ok: false, error: (p1 && p1.error) || 'unknown' }
          const data1 = toOfficialData(p1)
          // 段1 数据先同步(面板 60s 轮询/刷新即可拿到近期数据)
          syncOfficialToCache(data1)
          // 段2:后台抓剩余页,合并去重后落盘并更新缓存
          runOfficial([['OCGO_START_PAGE', '40']]).then((p2) => {
            if (!p2 || !p2.ok || !Array.isArray(p2.records) || !p2.records.length) return
            const merged = mergeOfficialRecords(data1.records, p2.records)
            const full = toOfficialData(Object.assign({}, p2, { records: merged }))
            writeOfficialDisk(merged)
            syncOfficialToCache(full)
          }).catch(() => {})
          return data1
        } catch (e) {
    ...
    async function fetchAll() {
      ...
      inflight = (async () => {
        // DSH 会话扫描与官方段1 拉取并行执行(互不等待);配额同时拉。
        // 回填是纯内存操作(backfillDsh),扫描只做一次。
        const scanP = collectDshScan().catch(() => [])
        ...
        if (!off) {
          off = await collectOfficial().catch(() => null)
        }
    ...
    async function launchDebugBrowser() {
      try {
        ...
        // 启动经 explorer.exe 中转(写临时 bat 后由桌面 explorer 运行):
        // explorer 是用户桌面已有进程,派生的浏览器不属于 DSH 的进程树,
        // 不会被 shell 服务在命令结束后清理,窗口正常显示;且只用 core cmdlet,
        // 不受受限执行环境(如 WMI cmdlet 被禁)影响。
        const ps = [
