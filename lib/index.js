...
export function apply(ctx) {
  ...
  "gemini-2.5-flash": { in: 0.3, out: 2.5, cr: 0.03, cw: 0.0 },
    ...
    // --- 数据源 5:官方账户级用量明细(usage.list server-fn) ---
    ...
    async function collectOfficial() {
      ...
      officialInflight = (async () => {
        try {
          ...
          return { ok: true, vd: buildView(rows), truncated: !!parsed.truncated, records: rows.length, autoExtracted: !!parsed.autoExtracted }
        } catch (e) {
