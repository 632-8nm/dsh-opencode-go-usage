...
import { writeFileSync as _ocgoWriteFileSync, mkdirSync as _ocgoMkdirSync, readFileSync as _ocgoReadFileSync, existsSync as _ocgoExistsSync, renameSync as _ocgoRenameSync, unlinkSync as _ocgoUnlinkSync } from 'node:fs'
import { join as _ocgoJoin } from 'node:path'
import { homedir as _ocgoHomedir } from 'node:os'
export function apply(ctx, config) {
    const sq = ctx.get('sessionQuery')
    if (sq === undefined) return
    ocgoLog('plugin loaded')
    ...
    async function collectQuota() {
      ...
      if (!key) { ocgoLog('quota: no opencode-go key'); return { error: 'no opencode-go key' } }
      const t0 = Date.now()
      ...
      for (let attempt = 0; attempt < 2; attempt++) {
        try {
          ...
          if (!res.ok) { ocgoLog('quota http ' + res.status + ' (attempt ' + attempt + ')'); return { error: 'http ' + res.status } }
          ...
          if (!Object.keys(windows).length) { ocgoLog('quota empty payload'); return { error: 'empty usage payload' } }
          ocgoLog('quota ok ' + Math.round(Date.now() - t0) + 'ms rolling=' + (windows.rolling ? windows.rolling.percent : '?') + '% weekly=' + (windows.weekly ? windows.weekly.percent : '?') + '% monthly=' + (windows.monthly ? windows.monthly.percent : '?') + '%')
          return { ok: true, keys: [{ name: 'default', active: true, error: null, windows }] }
        } catch (e) {
          ocgoLog('quota attempt ' + attempt + ' failed: ' + String((e && e.message) || e))
          if (attempt === 0) continue
          return { error: String((e && e.message) || e) }
        }
      }
      ocgoLog('quota query failed after retries')
    ...
    // 官方视图"最近会话":usage.list 没有会话标题,按时间回填的 DSH 会话补标题并
    // 按会话聚合(金额为官方回填值)。dshRows 为空且 DSH 扫描未就绪(dshLoading)
    // 时置空列表,避免退化为官方记录聚合显示一片"(无标题)"。
    function rebuildOfficialRecent(off, dshRows, dshLoading) {
      if (!off || !off.ok || !off.vd) return
      if (!dshRows || !dshRows.length) {
        if (dshLoading) off.vd.recent = []
        return
      }
    ...
    // DSH 会话扫描结果持久化:重启后恢复上次扫描行,DSH 视图秒开且后台增量重扫不空白。
    function dshScanPath() {
      if (typeof _ocgoJoin !== 'function' || typeof _ocgoHomedir !== 'function') return null
      return _ocgoJoin(_ocgoHomedir(), '.config', 'dsh-opencode-go-usage-dshscan.json')
    }
    function saveDshScan(rows) {
      const p = dshScanPath()
      if (!p || typeof _ocgoMkdirSync !== 'function') { ocgoLog('saveDshScan: no fs'); return }
      try {
        _ocgoMkdirSync(_ocgoJoin(_ocgoHomedir(), '.config'), { recursive: true })
        atomicJsonWrite(p, { at: Date.now(), rows })
      } catch (e) { ocgoLog('saveDshScan failed: ' + String((e && e.message) || e)) }
    }
    function restoreDshScan() {
      if (lastScan) return lastScan
      const p = dshScanPath()
      if (!p || typeof _ocgoReadFileSync !== 'function' || typeof _ocgoExistsSync !== 'function') return null
      try {
        if (!_ocgoExistsSync(p)) return null
        const d = JSON.parse(String(_ocgoReadFileSync(p, 'utf8') || '{}'))
        if (d && d.at && Array.isArray(d.rows)) {
          lastScan = { at: d.at, rows: d.rows }
          return lastScan
        }
      } catch (e) { ocgoLog('restoreDshScan failed: ' + String((e && e.message) || e)) }
      return null
    }
    ...
    async function runOfficial(envs) {
      ...
      let page = 0, skipped = 0, empty = 0, done = false, sampleText = ''
      try {
        while (page < maxPages && !done) {
          ...
          for (let i = 0; i < batch.length; i++) {
            ...
            if (!sampleText) sampleText = String(text).slice(0, 200).replace(/\s+/g, ' ')
        ...
        // 全量抓取但一条记录都没有:要么全部请求失败,要么响应里没有数据记录
        // (cookie 失效时 _server 常返回登录页 HTML)。不能当作"成功 0 条",
        // 否则面板显示全 0 且 15min 内不重试,用户无从知晓。增量模式无新记录
        // (LAST 过滤后为空)是正常情况,不在此列。
        if (!isIncremental && !out.records.length && page > 0) {
          out.ok = false
          out.error = skipped > 0
            ? 'official fetch failed: ' + skipped + ' page(s) failed'
            : 'empty usage.list response (cookie may be expired)'
          ocgoLog('official fetch empty: pages=' + page + ' skipped=' + skipped + ' sample=' + sampleText)
        } else {
          out.ok = true
          ocgoLog('official fetch ok: pages=' + page + ' records=' + out.records.length + ' skipped=' + skipped)
        }
        // 只有成功才落盘(错误/空结果不写,避免空缓存毒化增量基准)
        if (out.ok && canFs && diskPath) {
          try {
            _ocgoMkdirSync(_ocgoJoin(_ocgoHomedir(), '.config'), { recursive: true })
        ...
        out.truncated = out.ok && out.records.length >= maxPages * OCGO_PAGE_SIZE
    ...
    function saveOfficialConfig(payload) {
      try {
        ...
        // 删除磁盘快照:旧快照可能是保存前(无凭据/旧凭据)写的,official 为
        // NEED_CONFIG 或旧数据——不清掉的话保存后 reload 会命中它,把"加载中"
        // 顶成旧引导/旧数据。删掉后下次 fetchAll 直接走网络构建新数据。
        const sp = snapshotPath()
        if (sp && typeof _ocgoExistsSync === 'function' && typeof _ocgoUnlinkSync === 'function') {
          try { if (_ocgoExistsSync(sp)) _ocgoUnlinkSync(sp) } catch (e) { /* 删不掉也不阻塞保存 */ }
        }
        ocgoLog('credentials saved: workspaceId=' + (payload.workspaceId ? payload.workspaceId.slice(0, 8) + '…' : 'none'))
        return { ok: true }
      } catch (e) {
    ...
    // 配额结果缓存 + 并发去重:后台抓取,不阻塞首响应。
    // 首次使用/重启无缓存时,fetchAll 的响应毫秒级返回(面板/FAB 立刻展开),
    // 配额到位后通过 60s 轮询自然更新(本函数并发去重,不会重复请求)。
    let quotaCache = null
    let quotaInflight = null
    function refreshQuotaAsync() {
      if (quotaInflight) return quotaInflight
      ocgoLog('quota fetch start')
      quotaInflight = collectQuota()
        .then((q) => {
          const okQ = (q && !q.error) ? q : null
          quotaCache = okQ
          if (cache && cache.data) {
            cache.data.quota = okQ
            cache.data.quotaError = (q && q.error) || null
            cache.data.quotaLoading = !okQ
            // 配额到位即补写展示快照(配额不依赖官方明细:无 cookie 时官方为
            // NEED_CONFIG,快照依然记录配额与 DSH,重启后首屏即显示上次配额)
            if (okQ && cache.data.dsh) {
              writeSnapshot(cache.data)
            }
          }
          return q
        })
        .catch((e) => {
          ocgoLog('quota fetch failed: ' + String((e && e.message) || e))
          if (cache && cache.data) {
            cache.data.quota = null
            cache.data.quotaError = String((e && e.message) || e)
            cache.data.quotaLoading = true
          }
          return { error: String((e && e.message) || e) }
        })
    ...
    function refreshScanAsync() {
      ...
      const t0 = Date.now()
      ocgoLog('dsh scan start')
      scanInflight = collectDshScan()
        .then((rows) => {
          ocgoLog('dsh scan done in ' + Math.round(Date.now() - t0) + 'ms rows=' + rows.length)
          lastScan = { at: Date.now(), rows }
          saveDshScan(rows) // 持久化扫描结果,重启后恢复秒开 DSH 视图
          ...
          if (cache && cache.data) {
            ...
            // 扫描完成即重建官方视图"最近会话"标题——否则要等下一次 60s 轮询的
            // fetchAll 才恢复(fast-poll 在扫描完成时恰好停止,中间可能等 60s)。
            // 官方数据与 cache.data.official 是同一对象(syncOfficialToCache),直接生效。
            if (cache.data.official && cache.data.official.ok && cache.data.official.vd) {
              rebuildOfficialRecent(cache.data.official, dshRows, false)
            }
          }
        })
        .catch((e) => {
    ...
    async function refreshAllInBackground() {
      ...
      ocgoLog('bg refresh start')
      bgRefreshInflight = (async () => {
        cache = null
        try {
          await fetchAll(true)
          ocgoLog('bg refresh done')
        } catch (e) {
    ...
    async function fetchAll(forceRefresh) {
      ...
      if (!cache && !forceRefresh) {
        ...
        if (snap && snap.ok && snap.quota && snap.dsh) {
          // 标注旧数据(stale),前端可据此显示"数据来自上次,正在刷新"
          snap.stale = true
          // 快照配额恢复到内存缓存:否则后台刷新(fetchAll(true))会用空 quotaCache
          // 构建新响应,把刚秒显出来的配额覆写成 null(表现为"配额闪一下又消失")
          quotaCache = snap.quota
          cache = { at: Date.now(), data: snap }
          ocgoLog('snapshot hit: quota=' + (snap.quota.keys ? snap.quota.keys.length + ' key(s)' : 'none') + ' dsh=' + (snap.dsh ? 'yes' : 'no') + ' official=' + (snap.official && snap.official.ok ? 'ok' : (snap.official && snap.official.error) || 'missing'))
      ...
      inflight = (async () => {
        ...
        // DSH 会话扫描:优先恢复上次持久化结果(秒开),5 分钟内复用,过期则后台重扫。
        if (!lastScan) restoreDshScan()
        if (!(lastScan && Date.now() - lastScan.at < 5 * 60 * 1000)) {
          refreshScanAsync()
        }
        const dshRaw = lastScan ? lastScan.rows : []
        // 配额缓存为空(重启清空/冷启动/后台任务竞态)时,用磁盘快照配额打底:
        // 保证任何构建路径(含 bgRefresh 抢占构建)响应都带配额,面板配额区不空白;
        // 有快照则秒显旧配额,后台抓取完成后 60s 轮询更新为最新。
        if (!quotaCache) {
          const _snap = readSnapshot()
          if (_snap && _snap.quota) quotaCache = _snap.quota
        }
        // 配额后台抓取:不阻塞首响应——首次使用/重启无缓存时响应毫秒级返回(界面
        // 立刻展开、立即显示凭据引导或磁盘/快照数据),配额到位后由 60s 轮询更新。
        refreshQuotaAsync()
        ...
        // 官方视图"最近会话":补标题并聚合;扫描未就绪时置空,不显示"(无标题)"
        const dshLoading = !(lastScan && lastScan.rows)
        rebuildOfficialRecent(off, dshRows, dshLoading)
        const data = { ok: true, fetchedAt: Date.now(), quota: (quota && !quota.error) ? quota : null, quotaError: (quota && quota.error) || null, quotaLoading: !quota, dsh, dshLoading, official: off || officialErr || { ok: false, loading: true } }
        ...
        // 配额一到即写展示快照(配额不依赖官方明细:无 cookie 时官方 NEED_CONFIG
        // 也照写,重启后首屏先见上次配额与上次 DSH,后台再刷新官方与配额)。
        if (data.quota) {
          writeSnapshot(data)
        }
        return data
    ...
    const serve = async (arg) => {
      try {
        // 手动刷新带 force:跳过 45s 内存缓存与快照分支,强制重新构建最新数据。
        return await fetchAll(!!(arg && arg.force))
      } catch (e) {
        return { ok: false, error: String((e && e.message) || e) }
      }
    }
    ...
    if (ws !== undefined && typeof ws.register === 'function') {
      ctx.effect(() => ws.register({
        ...
        handler: async (req, res) => {
          try {
            // 手动刷新带 X-OCGO-FORCE:1 → fetchAll(force) 跳过缓存强制重抓
            const h = (req && req.headers) || {}
            const force = h['x-ocgo-force'] === '1' || h['X-OCGO-Force'] === '1'
