...
function apply(ctx, config) {
    ...
    const I18N = {
      zh: {
        ...
        'official.loading': '官方数据加载中(增量/全量抓取)…',
        'official.errNoBrowser': '完成一次配置后即可自动查看。教程:① 用浏览器打开并登录 opencode.ai ② 按 F12 → Application → Cookies 里找到 name 为 auth 的 cookie 值(F12 代码为 Fe26.2 开头)③ 在 usage 页面地址栏复制 wrk_xxx(workspaceId)④ 粘贴到下方并保存,下次自动读取。',
        'official.howto': '① 浏览器登录 opencode.ai\n② F12 → Application → Cookies\n③ 复制 name=auth 的 cookie 值(Fe26.2 开头)\n④ usage 页面地址栏的 wrk_xxx\n⑤ 粘贴下方并保存',
      ...
      en: {
        ...
        'official.loading': 'Loading official data (incremental/full fetch)…',
        'official.errNoBrowser': 'One-time setup, then automatic. How to: 1) open and sign in to opencode.ai in a browser  2) F12 → Application → Cookies  3) copy the cookie whose name is `auth` (starts with Fe26.2)  4) copy the wrk_xxx workspaceId from the usage page URL  5) paste both below and save — it loads automatically afterwards.',
        'official.howto': '1) Sign in to opencode.ai in your browser\n2) F12 → Application → Cookies\n3) Copy the cookie value named `auth` (starts with Fe26.2)\n4) Copy the wrk_xxx id from the usage page URL\n5) Paste below and save — it auto-loads next time',
        ...
        'resize.w': 'Drag to resize width',
        'resize.s': 'Drag to resize height',
        'resize.n': 'Drag to resize height',
        'resize.se': 'Drag to resize',
        'resize.ne': 'Drag to resize',
        'resize.nw': 'Drag to resize',
        'resize.sw': 'Drag to resize',
...
.ocgo-spacer { flex: 1; }
.ocgo-ibtn { background: none; border: none; cursor: pointer; color: var(--dsw-alias-label-secondary); display: inline-flex; align-items: center; justify-content: center; width: 24px; height: 24px; padding: 0; border-radius: 6px; transition: background .15s ease, color .15s ease; }
...
.ocgo-edge-n { position: absolute; left: 0; top: 0; width: 100%; height: 8px; cursor: ns-resize; }
.ocgo-edge-w { position: absolute; left: 0; top: 0; width: 8px; height: 100%; cursor: ew-resize; }
.ocgo-edge-ne { position: absolute; right: 0; top: 0; width: 16px; height: 16px; cursor: nesw-resize; z-index: 2; }
.ocgo-edge-nw { position: absolute; left: 0; top: 0; width: 16px; height: 16px; cursor: nwse-resize; z-index: 2; }
.ocgo-edge-sw { position: absolute; left: 0; bottom: 0; width: 16px; height: 16px; cursor: nesw-resize; z-index: 2; }
.ocgo-resize { position: absolute; right: 0; bottom: 0; width: 26px; height: 26px; cursor: nwse-resize; z-index: 2; }
...
`
    ...
    function Donut(props) {
      // rate-limited:窗口配额已耗尽(服务端限流),红色 + ⚠ 徽标,不显示倒计时
      const limited = props.status === 'rate-limited'
      ...
      return React.createElement('div', { className: 'ocgo-donut-wrap' },
        React.createElement('svg', { className: 'ocgo-donut', viewBox: '0 0 76 76' },
          ...
          React.createElement('circle', { cx: 38, cy: 38, r, fill: 'none', stroke: quotaColor(p, limited), strokeWidth: 9, strokeLinecap: 'round', strokeDasharray: (p / 100) * c + ' ' + c, transform: 'rotate(-90 38 38)', style: { transition: 'stroke-dasharray .5s ease' } }),
          React.createElement('text', { x: 38, y: 35, textAnchor: 'middle', className: 'ocgo-donut-val', fill: limited ? 'var(--dsw-alias-state-error-primary)' : undefined }, limited ? '⚠' : Math.round(p) + '%'),
          React.createElement('text', { x: 38, y: 48, textAnchor: 'middle', className: 'ocgo-donut-lbl' }, props.label)
        ),
        limited
          ? React.createElement('span', { className: 'ocgo-donut-time', style: { color: 'var(--dsw-alias-state-error-primary)', opacity: 1, fontWeight: 700 } }, props.t('donut.limited'))
    ...
    // 标题栏 SVG 图标(16×16 viewBox,stroke 风格,currentColor 随主题)
    function Icon({ name, size }) {
      const s = size || 14
      const common = { width: s, height: s, viewBox: '0 0 16 16', fill: 'none', stroke: 'currentColor', strokeWidth: 1.3, strokeLinecap: 'round', strokeLinejoin: 'round' }
      const paths = {
        // 语言切换:地球
        lang: [
          React.createElement('circle', { key: 'c', cx: 8, cy: 8, r: 6.2 }),
          React.createElement('path', { key: 'h', d: 'M1.8 8h12.4' }),
          React.createElement('path', { key: 'v', d: 'M8 1.8c2.3 1.9 2.3 10.5 0 12.4M8 1.8c-2.3 1.9-2.3 10.5 0 12.4' }),
        ],
        // 导出 CSV:下载箭头 + 托盘
        export: [
          React.createElement('path', { key: 'a', d: 'M8 2.8v7.4' }),
          React.createElement('path', { key: 'b', d: 'M4.8 7.2 8 10.4l3.2-3.2' }),
          React.createElement('path', { key: 'c', d: 'M2.5 13.2h11' }),
        ],
        // 刷新:环形箭头
        refresh: [
          React.createElement('path', { key: 'a', d: 'M13.6 5.4A5.6 5.6 0 1 0 13.6 10.6' }),
          React.createElement('path', { key: 'b', d: 'M13.6 1.9v3.5h-3.5' }),
        ],
        // 最大化:方块
        max: [React.createElement('rect', { key: 'r', x: 2.8, y: 2.8, width: 10.4, height: 10.4, rx: 1.5 })],
        // 还原:重叠方块
        restore: [
          React.createElement('path', { key: 'f', d: 'M5.8 5.8V2.8h7.4v7.4h-3' }),
          React.createElement('rect', { key: 'b', x: 2.8, y: 5.8, width: 7.4, height: 7.4, rx: 1.2 }),
        ],
        // 关闭:X
        close: [
          React.createElement('path', { key: 'x', d: 'M4.2 4.2l7.6 7.6M11.8 4.2l-7.6 7.6' }),
        ],
      }
      return React.createElement('svg', common, paths[name] || null)
    }
    ...
    function UsagePanel() {
      ...
      React.useEffect(() => {
        ...
        let disposer = null
        if (timer) disposer = timer.interval(load, 60000)
      ...
      function exportCsv() {
        try {
          ...
          if (d) {
            ...
            if (d.quota && Array.isArray(d.quota.keys) && d.quota.keys.length) {
              // 导出当前选中 key 的窗口(多 key 时每 key 一组)
              d.quota.keys.forEach((ke) => {
                const w = ke.windows
                if (!w) return
                push([])
                push(['配额窗口', 'key', '用量 %', '状态', '重置时间'])
                ;['rolling', 'weekly', 'monthly'].forEach((k) => {
                  const q = w[k]
                  if (q) push([k, ke.name, q.percent, q.status || '', q.resetsAt ? fmtTime(new Date(q.resetsAt).getTime()) : ''])
                })
              })
            }
      ...
      function onResizeDown(e, mode) {
        ...
        const startRight = rect.right, startBottom = rect.bottom
        // 边界:浏览器用 overlay 覆盖式滚动条(实测 sbW=0,不占布局宽度),
        // 滚动条浮在屏幕最右/最下边缘。为此右/下边界向内保留其浮出宽度(约17px),
        // 避免面板边缘压住滚动条。左/上无滚动条,边界为 0。
        const OVERLAY_SB = 9
        const maxRight = window.innerWidth - OVERLAY_SB
        const maxBottom = window.innerHeight - OVERLAY_SB
        const MIN_W = 300, MIN_H = 260
        const mE = mode.includes('e'), mW = mode.includes('w')
        const mS = mode.includes('s'), mN = mode.includes('n')
        let last = null
        const onMove = (ev) => {
          const dx = ev.clientX - sx
          const dy = ev.clientY - sy
          let left = startLeft, top = startTop
          let right = startRight, bottom = startBottom

          // 水平轴(严格钳制到 [0, maxRight])
          if (mE) right = Math.min(startRight + dx, maxRight)
          if (mW) left = Math.max(startLeft + dx, 0)
          // 保证最小宽度
          if (mW && right - left < MIN_W) left = right - MIN_W
          if (mE && right - left < MIN_W) right = left + MIN_W

          // 垂直轴(严格钳制到 [0, maxBottom])
          if (mS) bottom = Math.min(startBottom + dy, maxBottom)
          if (mN) top = Math.max(startTop + dy, 0)
          // 保证最小高度
          if (mN && bottom - top < MIN_H) top = bottom - MIN_H
          if (mS && bottom - top < MIN_H) bottom = top + MIN_H

          const next = { w: Math.floor(right - left), h: Math.floor(bottom - top) }
          const nextPos = { x: Math.floor(left), y: Math.floor(top) }
          last = { size: next, pos: nextPos }
          setSize(next)
          setPos(nextPos)
        }
        const onUp = () => {
          window.removeEventListener('mousemove', onMove)
          window.removeEventListener('mouseup', onUp)
          if (last) commitState({ size: last.size, pos: last.pos })
        }
        window.addEventListener('mousemove', onMove)
        window.addEventListener('mouseup', onUp)
      }
      function onFabDown(e) {
        ...
        const onUp = () => {
          ...
          window.removeEventListener('mouseup', onUp)
          if (!moved) {
            // 用实际尺寸(优先已保存的 size,否则默认)算展开位置,避免按默认 400 宽
            // 算的 pos 套上更大的实际 size,导致右/下缘超出屏幕。
            const w = (size && size.w) ? size.w : 400
            const h = (size && size.h) ? size.h : 560
      ...
      // 配额数据:多 key 池(host 返回 keys 数组)。当前 key = 手动选中 →
      // ACTIVE 标记 → 第一个;单 key 时行为与旧版一致。
      const quotaKeys = (d && d.quota && Array.isArray(d.quota.keys) && d.quota.keys.length) ? d.quota.keys : null
      const curQ = quotaKeys ? (quotaKeys.find((k) => k.name === qKey) || quotaKeys.find((k) => k.active) || quotaKeys[0]) : null
      const qz = curQ ? curQ.windows : null
      const rollPct = qz && qz.rolling ? qz.rolling.percent : null
      const fabTitle = t('title') + (curQ && curQ.name !== 'default' ? ' · ' + curQ.name : '') + ' · ' + t('fab.rolling', { p: rollPct != null ? pct(rollPct) : '—' }) +
      ...
      if (state.loading) {
        ...
        if (z && Object.keys(z).length) {
          // 多 key 池:key 切换标签行(★ = DSH 当前生效 key;⚠ = 该 key 查询失败)
          if (quotaKeys && quotaKeys.length > 1) {
            body.push(React.createElement('div', { key: 'keys', className: 'ocgo-viewrow', style: { flexWrap: 'wrap' } },
              quotaKeys.map((k) => React.createElement('button', {
                key: k.name,
                className: 'ocgo-seg-btn' + (curQ && curQ.name === k.name ? ' on' : ''),
                onClick: () => setQKey(k.name),
                title: k.error || (k.active ? t('fab.hint') : ''),
                style: { fontSize: 11 },
              }, (k.active ? '★ ' : '') + k.name + (k.error ? ' ⚠' : '')))
            ))
          }
          const donuts = []
          ...
          ;['rolling', 'weekly', 'monthly'].forEach((k) => {
            const q = z[k]
            if (q) donuts.push(React.createElement(Donut, { key: k, percent: q.percent, label: labels[k], resetsAt: q.resetsAt, status: q.status, t, pace: paceOf(q, Date.now(), windows[k]) }))
          })
        ...
        // 配额告警条:当前 key 任一窗口 ≥90% 时面板顶部醒目提示
        if (z) {
        ...
        if (vd.recent && vd.recent.length) {
          // 用 recent 内容指纹作 key:数据一变就整体重建列表,避免 React diff 复用
          // 旧 DOM 导致新旧两版 recent 叠加显示(金额/时间不同的同标题会话重复)。
          const recentFingerprint = vd.recent.map((s) => s.id + ':' + s.updated + ':' + s.cost_est).join('|')
          body.push(React.createElement('div', { key: 'recent-' + recentFingerprint, className: 'ocgo-panel2' },
        ...
        const qm = curQ && curQ.windows && curQ.windows.monthly
      ...
      return React.createElement('div', { className: 'ocgo-panel' + (maximized ? ' ocgo-max' : ''), style: panelStyle },
        ...
        React.createElement('div', { className: 'ocgo-edge-n', onMouseDown: (e) => onResizeDown(e, 'n'), title: t('resize.n') }),
        ...
        React.createElement('div', { className: 'ocgo-edge-w', onMouseDown: (e) => onResizeDown(e, 'w'), title: t('resize.w') }),
        React.createElement('div', { className: 'ocgo-edge-ne', onMouseDown: (e) => onResizeDown(e, 'ne'), title: t('resize.ne') }),
        React.createElement('div', { className: 'ocgo-edge-nw', onMouseDown: (e) => onResizeDown(e, 'nw'), title: t('resize.nw') }),
        React.createElement('div', { className: 'ocgo-edge-sw', onMouseDown: (e) => onResizeDown(e, 'sw'), title: t('resize.sw') }),
