export const name = "opencode-go-usage-client"
...
`)
    ...
    function UsagePanel() {
      ...
      function onTitleDown(e) {
        ...
        const onMove = (ev) => setPos({ x: bx + ev.clientX - sx, y: by + ev.clientY - sy })
      ...
      function onResizeDown(e, mode) {
        ...
        const onMove = (ev) => {
          ...
          setSize((prev) => {
            const p = prev || { w: bw, h: bh }
            const next = { w: p.w, h: p.h }
            if (mode === 'e' || mode === 'se') next.w = Math.max(300, bw + dx)
            if (mode === 's' || mode === 'se') next.h = Math.max(240, bh + dy)
            return next
          })
        }
        const onUp = () => { window.removeEventListener('mousemove', onMove); window.removeEventListener('mouseup', onUp) }
      ...
      function onFabDown(e) {
        ...
        const onMove = (ev) => {
          ...
          setFabPos({ x: bx + ev.clientX - sx, y: by + ev.clientY - sy })
        }
        const onUp = () => {
