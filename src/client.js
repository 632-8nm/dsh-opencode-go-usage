  ...
  apply(ctx) {
    ...
    const I18N = {
      zh: {
        ...
        'official.errEdge': 'Edge 正在运行,cookie 库被锁定。关闭 Edge 后点刷新即可自动提取,或手动粘贴下方凭据。',
        'official.errNoCookie': 'Edge 中未找到 opencode.ai 登录态,请先在 Edge 登录 opencode.ai 后重试。',
        'official.errNoEdge': '未找到 Edge 浏览器。可手动粘贴下方凭据。',
      ...
      en: {
        ...
        'official.errEdge': 'Edge is running and its cookie database is locked. Close Edge and refresh to auto-extract, or paste credentials below.',
        'official.errNoCookie': 'No opencode.ai login found in Edge. Sign in to opencode.ai in Edge first.',
        'official.errNoEdge': 'Edge not found. You can paste credentials below.',
...
`
    ...
    function UsagePanel() {
      ...
      if (state.loading) {
        ...
        const friendly = ec === 'EDGE_RUNNING' ? t('official.errEdge')
          : ec === 'NO_COOKIE' ? t('official.errNoCookie')
          : ec === 'NO_EDGE' ? t('official.errNoEdge')
        ...
        if (qm && qm.percent != null && vd && vd.month && vd.month.cost_est != null) {
          ...
          foot.push(React.createElement('span', {
            key: 'recon',
            className: 'ocgo-warn',
            style: Math.abs(diff) > officialEst * 0.15 ? {} : { color: 'var(--dsw-alias-state-success-primary)' },
