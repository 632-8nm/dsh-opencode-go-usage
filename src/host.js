...
return {
  apply(ctx, config) {
    ...
    // 用 ctx.subprocess 直接 spawn 浏览器(GUI 进程,不受 ACL 受限令牌影响)。
    // 返回 {ok:true} 表示已拉起;返回 null 表示此平台无候选或 subprocess 不可用,交由上层回退 shell。
    async function launchViaSubprocess(plat) {
      try {
        const winCands = [
        ...
        const needFilesystem = typeof _ocgoExistsSync === 'function'
        let exe = null
        let home = null
        if (plat === 'win32') {
          for (const c of winCands) {
            if (needFilesystem) { if (_ocgoExistsSync(c)) { exe = c; break } }
            else if (!c.includes('x86')) { exe = c; break }
          }
          home = (typeof _ocgoHomedir === 'function' && _ocgoHomedir()) || process.env.USERPROFILE || process.env.HOME || ''
        } else if (plat === 'darwin') {
          exe = '/Applications/Google Chrome.app/Contents/MacOS/Google Chrome'
          home = (typeof _ocgoHomedir === 'function' && _ocgoHomedir()) || process.env.HOME || ''
        } else if (plat === 'linux') {
          exe = null
          for (const b of ['google-chrome-stable', 'google-chrome', 'chromium', 'chromium-browser', 'microsoft-edge', 'brave-browser']) {
            if (needFilesystem) {
              const r = _ocgoExistsSync('/usr/bin/' + b)
              if (r) { exe = '/usr/bin/' + b; break }
            }
          }
          home = (typeof _ocgoHomedir === 'function' && _ocgoHomedir()) || process.env.HOME || ''
        }
        if (!exe) return null
        const udd = (plat === 'win32' && home) ? home + '\\.ocgo-browser-debug' : (home ? home + '/.ocgo-browser-debug' : null)
        const args = udd
          ? ['--remote-debugging-port=9222', '--user-data-dir=' + udd, 'https://opencode.ai']
          : ['--remote-debugging-port=9222', 'https://opencode.ai']
        // 不 await done:浏览器要常驻。spawn 不抛错即视为已拉起。
        const handle = subprocess.spawn({
          argv: [exe, ...args],
        ...
        if (!handle || handle.pid == null || handle.pid === -1) return null
        // fire-and-forget:防止未捕获 reject
    ...
    async function launchDebugBrowser() {
      try {
        ...
        const plat = (typeof process !== 'undefined' && process.platform) || ''
        // 优先走 ctx.subprocess(不受限)直接 spawn 浏览器。DSH 的 ctx.shell 经
        // ACL 受限令牌,explorer/MS Edge 这类 GUI 进程起不来;subprocess 是
        // MCP/LSP/subagent 等 spawn 真实进程的同一条通道,能正常起 GUI。
        if (subprocess && typeof subprocess.spawn === 'function') {
          const launched = await launchViaSubprocess(plat)
