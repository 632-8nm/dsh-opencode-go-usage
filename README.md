...
| 🏛️ | **官方账户级视图** | 直接调官网 `usage.list` API,逐请求官方计费(与官网账单一致,跨设备);凭据**自动从 Edge 提取**,零配置 |
...
│  (sessionQuery)      cache 读按会话增量计(实测单价)        │
├──────────────────────────────────────────────────────────┤
│  opencode 官方库     part 表 JOIN session,step-finish 逐  │
│  (opencode.db)       请求含官方 cost(仅 opencode-go)      │
├──────────────────────────────────────────────────────────┤
│  codex 会话记录      直读 ~/.codex/sessions/**/*.jsonl     │
│  (jsonl)             (不依赖 cc-switch;total_token_usage  │
│                      按官方定价计费,与 cc-switch 对账 <0.1%)│
├──────────────────────────────────────────────────────────┤
...
- **金额口径**:opencode / codex 为官方或代理记录的**真实 cost**;**DSH 部分按官方定价表估算**(输入/输出按官方单价;cache 读按**会话增量** × 实测单价 $0.031/M——直接求和会把累计快照重复计算,导致虚高 10 倍)
- **来源口径**:本地三源均**只统计 opencode-go 的流量**(DSH 按 `source.provider == 'opencode-go'` 过滤;opencode.db 按 `session.model.providerID == 'opencode-go'` 过滤;codex 的 `config.toml` 指向 `opencode.ai/zen/go/v1`,直读会话记录);deepseek 直连、opencode 免费模型等非 Go key 流量一律不计入
- **免费模型**:`*-free`(OpenCode Zen 免费)不计入
- **对账**:面板底部显示 `官方月 $X · 本地 $Y`;官方视图为账户级真相(跨设备、含本地已丢失的历史),本地视图为本机观察——两者缺口提示存在本地不可见流量(如 DSH 数据目录迁移前的会话)
...
**DSH 部分是估算,准吗?**
DSH 会话事件只有 token 没有 cost,按官方定价表估算:输入/输出走官方单价,cache 读按**会话增量** × 实测单价 $0.031/M(直接求和会把累计快照重复计算)。想精确请用"官方"视图(账户级真实计费)。
