...
test('host 源码包含官方 usage.list 抓取(配置/cookie/1e8 换算)', () => {
  ...
  // 自动提取
  assert.ok(src.includes('extract_cookie'), '应含浏览器 cookie 自动提取')
  assert.ok(src.includes('BROWSER_RUNNING'), '浏览器运行时应有明确错误码')
  assert.ok(src.includes('CryptUnprotectData'), '应使用 DPAPI 解密')
  assert.ok(src.includes('AESGCM'), '应使用 AES-GCM 解 cookie')
  assert.ok(src.includes('Firefox'), '应支持 Firefox')
  assert.ok(src.includes('Chrome'), '应支持 Chrome')
  assert.ok(src.includes('moz_cookies'), '应读 Firefox moz_cookies 表')
  ...
  assert.ok(client.includes("'official.errRunning'"), '面板应含浏览器运行提示')
